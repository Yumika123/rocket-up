...
@export var idle_swing_amount = 0.1  # Максимальный угол отклонения (в радианах)
@export var idle_swing_speed = 2.0  # Скорость раскачивания
...
var time = 0.0  # Переменная для синусоидального движения

var last_rotation = 0.0  # Последний угол перед отпусканием кнопки
...
func _process(delta):
	...
	if use_joystick:
		...
		if move_direction.length_squared() > 0:
			...
			# Эффект невесомости при отсутствии движения с джойстика
	...
	if is_touching:
		...
		# Эффект невесомости: колебание вокруг центрального положения
		rotation = last_rotation + sin(time * idle_swing_speed) * idle_swing_amount
